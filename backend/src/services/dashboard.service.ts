...
import peerValidationModel from "../models/peerValidation.model";
...
export default class DashboardService {
  async getStats(currentUserId: string): Promise<any> {
    const totalSpecialists = await userRepo
      .findAll(0, 0, UserRole.AGRI_SPECIALIST)
      .then((u) => u.length);
    const activeSpecialists = await userRepo
      .findAll(0, 0, UserRole.AGRI_SPECIALIST)
      .then((u) => u.filter((u: any) => u.is_active).length);
    const totalModerators = await userRepo
      .findAll(0, 0, UserRole.MODERATOR)
      .then((u) => u.length);
    const activeModerators = await userRepo
      .findAll(0, 0, UserRole.MODERATOR)
      .then((u) => u.filter((u: any) => u.is_active).length);
    ...
    const unreadNotifications = await notificationRepo.countUnreadByUserId(
      currentUserId
    );
  ...
  async getMyTasks(currentUserId: string, currentRole: string): Promise<any> {
    ...
    if (currentRole === UserRole.AGRI_SPECIALIST) {
      const assignedQuestions = await questionRepo.findAssignedToUser(
        currentUserId,
        [
      ...
      for (const question of assignedQuestions) {
        ...
        if (question.status === QuestionStatus.READY_FOR_GOLDEN_FAQ) {
          const currentAns = await answerRepo.findCurrentByQuestionId(
            question._id.toString()
          );
          if (
            currentAns &&
            currentAns.specialist_id.toString() === currentUserId
          )
            taskType = "create_golden_faq";
      ...
      const peerNotifications = await notificationRepo.findAllByUserId(
        currentUserId
      );
      
      for (const notification of peerNotifications) {
        const peerAnswer = await answerRepo.findByAnswerId(
  ...
  // 🧠 Check if this user has already submitted a peer validation for this answer
  const alreadyReviewed = await peerValidationModel.findOne({
    answer_id: peerAnswer._id,
    reviewer_id: new Types.ObjectId(currentUserId),
  });

  // If already reviewed, skip adding this task
  ...
  tasks.push({
    ...
    question_text: q.original_query_text,
    ...
    district: q.district || 'N/A',
        ...
        // if (
        //   peerAnswer &&
        //   peerAnswer.question_id &&
        //   !(peerAnswer.question_id instanceof Types.ObjectId)
        // ) {
        //   const q = peerAnswer.question_id as IQuestion;
      ...
      const validationNotifications = await notificationRepo.findAllByUserId(
        currentUserId
      );
      for (const notification of validationNotifications) {
        const answer = await answerRepo.findByAnswerId(
          notification.related_entity_id as string
        );
        if (
          answer &&
          answer.question_id &&
          !(answer.question_id instanceof Types.ObjectId)
        ) {
          ...
          if (q.status === QuestionStatus.PENDING_MODERATION) {
            tasks.push({
              ...
              question_id: q.question_id,
              question_text: q.original_query_text,
              answer_preview: answer.answer_text,
    ...
    const validationNotifications = await peerValidation.findUnreadByUserId(
      currentUserId,
      QuestionStatus.ASSIGNED_TO_MODERATION
    );
    ...
    if (validationNotifications && validationNotifications.length > 0) {
      const questionList = await Promise.all(
        validationNotifications.map(async (notif) => {
          ...
          const questionObj = await questionRepo.findByQuestionId(
            notif.quetion_id
          );
    ...
    const validationNotificationsRject =
      await peerValidation.findUnreadByUserId(
        currentUserId,
        QuestionStatus.QUESTION_SENDBACK_TO_OWNER
      );

    if (
      validationNotificationsRject &&
      validationNotificationsRject.length > 0
    ) {
      const questionList = await Promise.all(
    ...
    const rejectedAnswers = await answerRepo.findRejectedQuestions(
      currentUserId,
      status,
      revisionSuccess
    );
    if (rejectedAnswers) {
      await Promise.all(
        rejectedAnswers.map(async (answer) => {
          const questionObj = await questionRepo.findByQuestionObjectId(
            answer.question_id
          );
          const peerValidationObj = await peerValidation.findByAnswerObjId(
            answer._id
          );
  ...
  async getUserPerformance(
    ...
    const userPerformance = await peerValidation.findByReviewerId(
      currentUserId
    );
  ...
  async updateUserState(
    ...
    const userPerformance = await userRepo.updateUserState(
      currentUserId,
      locationDetails
    );
