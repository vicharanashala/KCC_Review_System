...
const isValidJwtExpire =
  /^[0-9]+[smhdwy]$/.test(jwtExpire) || !isNaN(Number(jwtExpire));
if (!isValidJwtExpire) {
  throw new Error(
    'JWT_EXPIRE must be a number or a string with a valid time unit (e.g., "1h", "1d", "3600")'
  );
}

export const config = {
  mongoUri:
    process.env.MONGO_URI || "mongodb://localhost:27017/kcc_review_system",
